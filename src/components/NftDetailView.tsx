'use client'

import Link from 'next/link'
import { useRouter } from 'next/navigation'
import { useState } from 'react'
import {
  useAccount,
  useReadContract,
  useWriteContract,
  useWaitForTransactionReceipt,
} from 'wagmi'
import { parseUnits, isAddress } from 'viem'
import {
  marketplaceABI,
  marketplaceAddress,
  nftABI,
  nftAddress,
  tokenABI,
  tokenAddress,
} from '@/src/app/contracts'
import { useNftMetadata } from '@/src/hooks/useNftMetadata'
import { formatTokenAmount, shortenAddress } from '@/src/lib/format'
import { Header } from '@/src/components/Header'
import { BuyNftButton } from '@/src/components/nft/BuyNftButton'
import { ListNftForm } from '@/src/components/nft/ListNftForm'
import { CancelListingButton } from '@/src/components/nft/CancelListingButton'

type NftDetailViewProps = {
  tokenId: bigint
  from?: string
}

export function NftDetailView({ tokenId, from }: NftDetailViewProps) {
  const router = useRouter()
  const { address, isConnected } = useAccount()

  // offerPrice 상태
  const [offerAmount, setOfferAmount] = useState('')
  const [offerError, setOfferError] = useState('')

  // acceptOffer 상태
  const [buyerAddress, setBuyerAddress] = useState('')
  const [acceptError, setAcceptError] = useState('')

  const { data: tokenUri } = useReadContract({
    address: nftAddress,
    abi: nftABI,
    functionName: 'tokenURI',
    args: [tokenId],
  })

  const { data: owner } = useReadContract({
    address: nftAddress,
    abi: nftABI,
    functionName: 'ownerOf',
    args: [tokenId],
  })

  const { data: listing, refetch: refetchListing } = useReadContract({
    address: marketplaceAddress,
    abi: marketplaceABI,
    functionName: 'getListing',
    args: [tokenId],
  })

  const { data: decimals } = useReadContract({
    address: tokenAddress,
    abi: tokenABI,
    functionName: 'decimals',
  })

  const { data: symbol } = useReadContract({
    address: tokenAddress,
    abi: tokenABI,
    functionName: 'symbol',
  })

  // 제안 금액 조회 (buyerAddress 입력 시)
  const { data: offerData } = useReadContract({
    address: marketplaceAddress,
    abi: marketplaceABI,
    functionName: 'offers',
    args: isAddress(buyerAddress)
      ? [tokenId, buyerAddress as `0x${string}`]
      : undefined,
    query: { enabled: isAddress(buyerAddress) },
  })

  const { metadata, imageUrl, loading } = useNftMetadata(tokenUri)

  const tokenDecimals = decimals ?? 18
  const tokenSymbol = symbol ?? 'TOKEN'

  const isListed = listing?.[2] ?? false
  const listPrice = listing?.[0]
  const seller = listing?.[1]

  const isOwner =
    address && owner && owner.toLowerCase() === address.toLowerCase()

  const isSeller =
    address &&
    seller &&
    isListed &&
    seller.toLowerCase() === address.toLowerCase()

  const backHref =
    from === 'my' ? '/?tab=my' : from === 'mint' ? '/?tab=mint' : '/?tab=market'

  const handleSuccess = () => {
    refetchListing()
    router.refresh()
  }

  // offerPrice 훅
  const {
    writeContractAsync: writeOffer,
    isPending: isOfferPending,
    data: offerHash,
  } = useWriteContract()
  const { isLoading: isOfferConfirming, isSuccess: isOfferSuccess } =
    useWaitForTransactionReceipt({ hash: offerHash })

  // acceptOffer 훅
  const {
    writeContractAsync: writeAccept,
    isPending: isAcceptPending,
    data: acceptHash,
  } = useWriteContract()
  const { isLoading: isAcceptConfirming, isSuccess: isAcceptSuccess } =
    useWaitForTransactionReceipt({ hash: acceptHash })

  const handleOffer = async () => {
    if (!offerAmount) return
    try {
      setOfferError('')
      const amount = parseUnits(offerAmount, tokenDecimals)
      await writeOffer({
        address: marketplaceAddress,
        abi: marketplaceABI,
        functionName: 'offerPrice',
        args: [tokenId, amount],
      })
    } catch (e) {
      setOfferError(e instanceof Error ? e.message : String(e))
    }
  }

  const handleAccept = async () => {
    if (!isAddress(buyerAddress)) {
      setAcceptError('올바른 주소를 입력해주세요.')
      return
    }
    try {
      setAcceptError('')
      await writeAccept({
        address: marketplaceAddress,
        abi: marketplaceABI,
        functionName: 'acceptOffer',
        args: [tokenId, buyerAddress as `0x${string}`],
      })
      handleSuccess()
    } catch (e) {
      setAcceptError(e instanceof Error ? e.message : String(e))
    }
  }

  return (
    <div className="flex min-h-full flex-col">
      <Header />
      <main className="mx-auto w-full max-w-4xl flex-1 px-4 py-8">
        <Link
          href={backHref}
          className="mb-6 inline-flex text-sm text-violet-600 hover:text-violet-500 dark:text-violet-400"
        >
          ← 목록으로
        </Link>

        <div className="grid gap-8 md:grid-cols-2">
          <div className="overflow-hidden rounded-xl border border-zinc-200 bg-zinc-100 dark:border-zinc-800 dark:bg-zinc-800">
            {loading ? (
              <div className="flex aspect-square items-center justify-center text-zinc-500">
                로딩 중…
              </div>
            ) : imageUrl ? (
              <img
                src={imageUrl}
                alt={metadata?.name ?? `NFT #${tokenId}`}
                className="aspect-square w-full object-cover"
              />
            ) : (
              <div className="flex aspect-square items-center justify-center text-zinc-500">
                이미지 없음
              </div>
            )}
          </div>

          <div className="flex flex-col gap-4">
            <div>
              <h1 className="text-2xl font-bold">
                {metadata?.name ?? `NFT #${tokenId.toString()}`}
              </h1>
              <p className="mt-1 text-sm text-zinc-500">
                Token ID: {tokenId.toString()}
              </p>
            </div>

            {metadata?.description && (
              <p className="text-zinc-600 dark:text-zinc-400">
                {metadata.description}
              </p>
            )}

            <dl className="space-y-2 rounded-xl border border-zinc-200 p-4 text-sm dark:border-zinc-800">
              <div className="flex justify-between gap-4">
                <dt className="text-zinc-500">소유자</dt>
                <dd className="font-mono">
                  {owner ? shortenAddress(owner, 6) : '—'}
                </dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-zinc-500">판매 상태</dt>
                <dd>{isListed ? '판매 중' : '미등록'}</dd>
              </div>
              {isListed && listPrice !== undefined && listPrice > BigInt(0) && (
                <div className="flex justify-between gap-4">
                  <dt className="text-zinc-500">가격</dt>
                  <dd className="font-medium text-violet-600 dark:text-violet-400">
                    {formatTokenAmount(listPrice, tokenDecimals, tokenSymbol)}
                  </dd>
                </div>
              )}
              {isListed && seller && (
                <div className="flex justify-between gap-4">
                  <dt className="text-zinc-500">판매자</dt>
                  <dd className="font-mono">{shortenAddress(seller, 6)}</dd>
                </div>
              )}
            </dl>

            <div className="mt-2 border-t border-zinc-200 pt-6 dark:border-zinc-800 flex flex-col gap-4">
              {isListed && !isSeller && (
                <BuyNftButton
                  tokenId={tokenId}
                  price={listPrice!}
                  seller={seller!}
                  address={address}
                  isConnected={isConnected}
                  onSuccess={handleSuccess}
                />
              )}

              {isSeller && (
                <CancelListingButton
                  tokenId={tokenId}
                  onSuccess={handleSuccess}
                />
              )}

              {isOwner && !isListed && (
                <ListNftForm
                  tokenId={tokenId}
                  tokenDecimals={tokenDecimals}
                  address={address}
                  onSuccess={handleSuccess}
                />
              )}

              {/* 추가: 구매 희망 가격 제안 (판매 중이고 판매자가 아닐 때) */}
              {isListed && !isSeller && isConnected && (
                <div className="rounded-xl border border-zinc-200 p-4 dark:border-zinc-800">
                  <h3 className="mb-3 text-sm font-semibold">가격 제안</h3>
                  <div className="flex gap-2">
                    <input
                      type="number"
                      value={offerAmount}
                      onChange={(e) => setOfferAmount(e.target.value)}
                      placeholder={`제안 가격 (${tokenSymbol})`}
                      className="flex-1 rounded-lg border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-600 dark:bg-zinc-800"
                    />
                    <button
                      onClick={handleOffer}
                      disabled={
                        isOfferPending || isOfferConfirming || !offerAmount
                      }
                      className="rounded-lg bg-violet-600 px-4 py-2 text-sm font-medium text-white hover:bg-violet-500 disabled:opacity-50"
                    >
                      {isOfferPending
                        ? '서명 대기...'
                        : isOfferConfirming
                          ? '처리중...'
                          : isOfferSuccess
                            ? '제안 완료 ✓'
                            : '제안하기'}
                    </button>
                  </div>
                  {offerError && (
                    <p className="mt-2 text-xs text-red-500">{offerError}</p>
                  )}
                </div>
              )}

              {/* 추가: 제안 수락 (판매자일 때) */}
              {isSeller && (
                <div className="rounded-xl border border-zinc-200 p-4 dark:border-zinc-800">
                  <h3 className="mb-3 text-sm font-semibold">제안 수락</h3>
                  <div className="flex flex-col gap-2">
                    <input
                      type="text"
                      value={buyerAddress}
                      onChange={(e) => setBuyerAddress(e.target.value)}
                      placeholder="구매자 주소 (0x...)"
                      className="rounded-lg border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-600 dark:bg-zinc-800"
                    />
                    {offerData !== undefined && offerData > BigInt(0) && (
                      <p className="text-sm text-violet-600">
                        제안 금액:{' '}
                        {formatTokenAmount(
                          offerData,
                          tokenDecimals,
                          tokenSymbol,
                        )}
                      </p>
                    )}
                    {offerData !== undefined &&
                      offerData === BigInt(0) &&
                      isAddress(buyerAddress) && (
                        <p className="text-sm text-zinc-500">
                          이 주소의 제안이 없습니다.
                        </p>
                      )}
                    <button
                      onClick={handleAccept}
                      disabled={
                        isAcceptPending ||
                        isAcceptConfirming ||
                        !isAddress(buyerAddress) ||
                        !offerData ||
                        offerData === BigInt(0)
                      }
                      className="rounded-lg bg-green-600 px-4 py-2 text-sm font-medium text-white hover:bg-green-500 disabled:opacity-50"
                    >
                      {isAcceptPending
                        ? '서명 대기...'
                        : isAcceptConfirming
                          ? '처리중...'
                          : isAcceptSuccess
                            ? '수락 완료 ✓'
                            : '제안 수락하기'}
                    </button>
                    {acceptError && (
                      <p className="text-xs text-red-500">{acceptError}</p>
                    )}
                  </div>
                </div>
              )}

              {isListed && !isSeller && !isConnected && (
                <p className="text-sm text-zinc-500">
                  지갑을 연결하면 구매할 수 있습니다.
                </p>
              )}

              {!isListed && !isOwner && isConnected && (
                <p className="text-sm text-zinc-500">
                  이 NFT는 현재 판매되지 않습니다.
                </p>
              )}
            </div>
          </div>
        </div>
      </main>
    </div>
  )
}
